    ...
    <div className="bg-background text-foreground">
      ...
      <div className="border-b border-border bg-surface/30 px-4 py-3 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl flex items-center gap-2 text-xs font-medium text-muted-foreground">
          <Link to="/courses" className="hover:text-primary transition-colors">Explore</Link>
          <ChevronRight className="h-3 w-3" />
          <Link to="/browse/$category" params={{ category: categorySlug }} className="hover:text-primary transition-colors">
            {categoryName}
          </Link>
          <ChevronRight className="h-3 w-3" />
        ...
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto">
            ...
            <p className="mt-4 text-muted-foreground leading-relaxed">
              Master the specific skills needed for {subcategoryName}. Compare top-rated certificates and specializations.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <Section className="py-16">
        {subcategoryCourses.length === 0 ? (
          <div className="text-center py-20 bg-surface/30 rounded-2xl border border-dashed border-border/60">
            <h3 className="text-lg font-semibold mb-2">No courses specifically for "{subcategoryName}" yet.</h3>
            <p className="text-muted-foreground">Try broadening your search to the parent category.</p>
            <Button asChild className="mt-6">
              <Link to="/browse/$category" params={{ category: categorySlug }}>Back to {categoryName}</Link>
            </Button>
          </div>
        ) : (
              ...
              >
                <div>
                  <div className="flex items-start justify-between mb-4">
                    <Badge variant="outline" className={`border-${course.accent}-500/30 text-${course.accent}-500 bg-${course.accent}-500/10`}>
                  ...
                  <div className="flex items-center gap-3 text-xs text-muted-foreground mb-6">
                    <span className="flex items-center"><Star className="mr-1 h-3.5 w-3.5 text-amber-500 fill-current" /> {course.rating || 4.8}</span>
                    <span className="flex items-center"><Clock className="mr-1 h-3.5 w-3.5" /> {course.duration}</span>
                  </div>
                </div>
